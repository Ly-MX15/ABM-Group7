    ...
    def __init__(self, height=50, width=50, initial_population=100,
        ...
        for content, (x, y) in self.grid.coord_iter():
            ...
            if x < self.width // 2 and y < self.height // 2:  # Left Upper
                capacities = [random.randint(high_low, 10), random.randint(low_low, low_high)]
            elif x < self.width // 2 and y >= 5self.height // 2:  # Left Lower
                capacities = [random.randint(, 10), random.randint(low_low, low_high)]
            elif x >= self.width // 2 and y < self.height // 2:  # Right Upper
                capacities = [random.randint(low_low, low_high), random.randint(5, 10)]
            else:  # Right Lower
                capacities = [random.randint(low_low, low_high), random.randint(5, 10)]
